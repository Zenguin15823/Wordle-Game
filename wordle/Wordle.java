...
public class Wordle extends JPanel implements KeyListener, MouseListener {

	private String word = "guess";
	private String letters = "qwertyuiopasdfghjklzxcvbnm";
	private Box[][] grid = new Box[6][5];
	private String[] guesses = new String[6];
	private Box[] alpha = new Box[26];
	private int[] distrib = new int[6];
	private int wins = 0;
	private boolean the = false;
	private Color[] light = {Color.WHITE, Color.DARK_GRAY, Color.LIGHT_GRAY, Color.BLACK, Color.DARK_GRAY}; //background, wordboxes, letterboxes, wordboxes fill
	private Color[] dark = {Color.BLACK, Color.DARK_GRAY, Color.LIGHT_GRAY, Color.WHITE, Color.DARK_GRAY};
	private Color[] woods = {Color.RED, Color.YELLOW, Color.LIGHT_GRAY, Color.YELLOW, Color.DARK_GRAY};
	private int money;
	private boolean[] easter = new boolean[5]; //0-jesus, 1-woodstheme, 2-wordle, 3-urmom 4-armstrong

	public static void main(String[] args) {
		JFrame frame = new JFrame("Wordle");
		Wordle canvas = new Wordle();
	...
	private void setUp() {
		for (int x = 0; x < 10; x++)
			alpha[x] = new Box(x*35 + 185, 630, 3, letters.charAt(x));
		for (int x = 10; x < 19; x++)
			alpha[x] = new Box(x*35 - 145, 665, 3, letters.charAt(x));
		for (int x = 19; x < 26; x++)
			alpha[x] = new Box(x*35 - 425, 700, 3, letters.charAt(x));

		for (int x = 0; x < grid.length; x++)
			for (int y = 0; y < grid[x].length; y++)
				grid[x][y] = new Box((y%6)*65 + 200, x*65 + 170, 60, 60, 0, ' ');

		int num = (int)(Math.random()*5757);
		word = wordlist[num].toLowerCase().strip();
		curguess = 0;
		won = false;
		money = 0;
		easter[4] = false;
	...
	public void paintComponent(Graphics g) {
		...
		if (easter[1]) theme = woods;
		super.paintComponent(g);
		super.setBackground(theme[0]);
		Graphics2D g2d = (Graphics2D) g;

		g2d.setFont(new Font("Times New Roman", 0, 20));
		g2d.setColor(theme[3]);
		if (easter[4]) g2d.drawString("aa Aaa Aaaa aaa Aaaa Aaaa", 250, 155);
		else g2d.drawString("by Zac Hays and Adah Oche", 250, 155);

		g2d.setFont(new Font("Arial", 1, 100));
		if (easter[4]) g2d.drawString("Aaaaaa", 190, 130);
		else if (easter[0]) g2d.drawString("Jesus", 218, 130);
		else if (title.length() > 0) g2d.drawString(title, 190, 130);
		else g2d.drawString("Wordle", 190, 130);

		g2d.setFont(new Font("Arial", 0, 18));
		if (easter[4]) g2d.drawString("Aaaaaa Aaaaa", 580, 685);
		else g2d.drawString("Switch Theme", 580, 685);

		g2d.setFont(new Font("Comic Sans MS", 1, 15));
		g2d.setColor(Color.GREEN);
		g2d.drawString("Cashapp us at $AJOche", 5, 690);
		g2d.drawString("(We deserve it)", 5, 705);
		g2d.setFont(new Font("Comic Sans MS", 0, money));
		if (easter[4]) g2d.drawString("aaaaa", 10, 675);
		else g2d.drawString("money", 10, 675);


		for (Box b : alpha) {
			if (b.color == 0) g2d.setColor(theme[4]);
			else if (b.color == 1) g2d.setColor(new Color(199, 193, 12));
			else if (b.color == 2) g2d.setColor(Color.GREEN);
			else if (b.color == 3) g2d.setColor(theme[2]);
			g2d.fillRect(b.x, b.y, b.w, b.h);
			g2d.setColor(theme[3]);
			g2d.setFont(new Font("Arial", 1, b.w-5));
			g2d.drawString("" + b.letter, b.x+5, b.y+b.h-5);
		}
		g2d.setColor(theme[2]);
		g2d.fillRect(175, 700, 60, 30);
		g2d.fillRect(485, 700, 60, 30);
		g2d.setColor(theme[3]);
		g2d.setFont(new Font("Arial", 1, 17));
		if (easter[4]) {
			g2d.drawString("AAAAA", 176, 722);
			g2d.drawString("AAAA", 491, 722);
		} else {
			g2d.drawString("ENTER", 176, 722);
			g2d.drawString("BKSP", 491, 722);
		}
		for (Box[] bb : grid) 
			for (Box b : bb) {
				if (b.color == 0) g2d.setColor(theme[0]);
				else if (b.color == 1) g2d.setColor(new Color(199, 193, 12));
				else if (b.color == 2) g2d.setColor(Color.GREEN);
				else if (b.color == 3) g2d.setColor(theme[4]);
				g2d.fillRect(b.x, b.y, b.w, b.h);
				if (b.color == 0) {
				...
				g2d.setColor(theme[3]);
				g2d.setFont(new Font("Arial", 1, b.w-5));
				g2d.drawString("" + b.letter, b.x+5, b.y+b.h-10);
			}

		if (easter[3]) {
			g2d.setColor(Color.WHITE);
			g2d.fillRect(0, 0, 850, 800);
			g2d.setColor(Color.YELLOW);
			g2d.fillOval(275, 90, 250, 400);
			g2d.setColor(Color.RED);
			g2d.fillOval(150, 280, 500, 500);
			g2d.setColor(Color.ORANGE);
			g2d.fillOval(315, 135, 175, 175);
			g2d.setColor(Color.WHITE);
			g2d.fillOval(340, 185, 50, 50);
			g2d.fillOval(420, 185, 50, 50);
			g2d.setColor(Color.BLACK);
			g2d.fillOval(357, 202, 20, 20);
			g2d.fillOval(437, 202, 20, 20);
			g2d.fillOval(350, 240, 100, 50);
			g2d.fillRect(0, 0, 850, 150);
			g2d.fillRect(0, 620, 850, 150);
			g2d.setColor(Color.WHITE);
			g2d.setFont(new Font("Impact", 1, 100));
			if (easter[4]) {
				g2d.drawString("aaaa aaa aa a", 110, 115);
				g2d.drawString("aaaaa", 270, 730);
			} else {
				g2d.drawString("your mom is a", 110, 115);
				g2d.drawString(word, 270, 730);
			}
		}

		if (won || curguess >= 6) {
			g2d.setColor(theme[4]);
			g2d.fillRect(150, 225, 420, 350);
			g2d.setColor(theme[3]);
			g2d.setFont(new Font("Impact", 0, 36));
			if (won) g2d.drawString("YOU WON!!!! Word was: " + word, 155, 280);
			else g2d.drawString("YOU LOST!!! Word was: " + word, 155, 280);
			g2d.setFont(new Font("Times New Roman", 0, 42));
			g2d.drawString("Guess Distribution", 200, 330);
			g2d.setFont(new Font("Times New Roman", 0, 24));
			for (int x = 0; x < distrib.length; x++) {
				int j = wins;
				if (j == 0) j = 1;
				g2d.drawString("" + (x+1), 190, (x*30) + 368);
				g2d.fillRect(210, (x*30) + 348, (270/j)*distrib[x] + 20, 25);
				g2d.drawString("" + distrib[x], 240 + (270/j)*distrib[x], (x*30) + 368);
			}
			g2d.setFont(new Font("Impact", 0, 28));
			g2d.drawString("Click or Press Enter to Play Again", 153, 569);
		}
	}
	...
	private void logic(String guess) {
		...
		if (curguess < 6) {
			guesses[curguess] = guess;
			calculate(guess, curguess);
			if (guess.strip().toLowerCase().equals("jesus")) easter[0] = true;
			if (guess.strip().toLowerCase().equals("urmom")) easter[3] = true;
			if (guess.strip().toLowerCase().equals(word)) {
				won = true;
				distrib[curguess]++;
				wins++;
			}
	...
	private void calculate(String worb, int index) {
		int[] wows = new int[5];
		for (int x = 0; x < 5; x++) wows[x] = 3;
		char[] wordar = word.toCharArray();
		for (int x = 0; x < 5; x++) {
			if (alpha[letters.indexOf(worb.charAt(x))].color != 2)
				alpha[letters.indexOf(worb.charAt(x))].color = 0;

			if (worb.charAt(x) == wordar[x]) {
				wows[x] = 2;
				alpha[letters.indexOf(worb.charAt(x))].color = 2;
				wordar[x] = '_';
			}
		}
		for (int x = 0; x < 5; x++)
			for (int y = 0; y < 5; y++)
				if (wows[x] != 2 && worb.charAt(x) == wordar[y]) {
					wows[x] = 1;
					if (alpha[letters.indexOf(worb.charAt(x))].color != 2)
						alpha[letters.indexOf(worb.charAt(x))].color = 1;
					wordar[y] = '_';
					break;
				}

		for (int x = 0; x < 5; x++) {
			grid[index][x].color = wows[x];
			grid[index][x].letter = worb.charAt(x);
		}
	}
	...
	/*private void printer() {
		System.out.println("----------");
		for (int x = 0; x < 6; x++) {
			System.out.println(guesses[x]);
			for (int w : guessesWow[x]) System.out.print(w + " ");
			System.out.println();
		}
		System.out.println("----------");
	}*/
	...
	public void keyPressed(KeyEvent e) {
		...
		temp += e.getKeyChar();
		if (temp.length() > 20) temp = temp.substring(temp.length()-20, temp.length());

		if (temp.contains("212")) {
			easter[1] = !easter[1];
			temp = "";
		}
		if (temp.toLowerCase().contains("wordle")) {
			easter[2] = true;
			easter[0] = false;
			temp = "";
		}
		if (temp.toLowerCase().contains("armstrong")) {
			easter[4] = true;
			temp = "";
		}

		if (won || curguess >= 6) {
			if (e.getKeyCode() == KeyEvent.VK_ENTER) setUp();
		}
		else {

			if (e.getKeyCode() == KeyEvent.VK_BACK_SPACE && !typed.isEmpty()) typed = typed.substring(0, typed.length()-1);
			else if (e.getKeyCode() == KeyEvent.VK_ENTER) {

				if (easter[3]) easter[3] = false;

				boolean cool = false;

				for (int x = 0; x < wordlist.length; x++)
					if (wordlist[x].strip().equals(typed.strip().toLowerCase())) cool = true;

				if (cool) {
					if (easter[2]) title = ("" + typed.charAt(0)).toUpperCase() + typed.substring(1);
					easter[2] = false;
					logic(typed);
					typed = "";
				}
				//else System.out.println("Invalid word! Guess again.");
			}
			else if (e.getKeyCode() == KeyEvent.VK_UP || e.getKeyCode() == KeyEvent.VK_DOWN) the = !the;
			else if (e.getKeyChar() == '$') {
				money++;
				if (money >= 20 && !easter[4]) word = "money";
			}
			else if (typed.length() < 5) typed += e.getKeyChar();
			typed = typed.toLowerCase().replaceAll("[^a-z]", "");
			//System.out.println(typed);
			if(curguess < 6) {
				for (int x = 0; x < typed.length(); x++)
					grid[curguess][x].letter = typed.charAt(x);
				for (int x = typed.length(); x < 5; x++)
					grid[curguess][x].letter = ' ';
			}
			if (easter[4]) {
				for (Box a : alpha) {
					a.letter = 'a';
	...
	public void mousePressed(MouseEvent e) {
		...
		if (won || curguess >= 6) {
			...
			for (int x = 0; x < alpha.length; x++) {
				if (ex >= alpha[x].x && ex <= alpha[x].x + alpha[x].w && ey >= alpha[x].y && ey <= alpha[x].y + alpha[x].h) {
					if (typed.length() < 5) typed += alpha[x].letter;
				}
			}
			if (ex >= 175 && ex <= 235 && ey >= 700 && ey <= 730) {
				boolean cool = false;

				for (int b = 0; b < wordlist.length; b++)
					if (wordlist[b].strip().equals(typed.strip().toLowerCase())) cool = true;
			...
			else if (ex >= 485 && ex <= 545 && ey >= 700 && ey <= 730 && !typed.isEmpty()) typed = typed.substring(0, typed.length()-1);
			else if (ex >= 112 && ex <= 127 && ey >= 677 && ey <= 692) {
				money++;
				if (money >= 20) word = "money";
			}
			else if (ex >= 580 && ex <= 695 && ey >= 669 && ey <= 691) the = !the;

			//g2d.drawRect(112, 677, 15, 15);
	...
	private class Box {
		...
		private Box(int xx, int yy, int col, char let) {
			x = xx;
			y = yy;
			w = 30;
			h = 30;
		...
		private Box(int xx, int yy, int ww, int hh, int col, char let) {
			x = xx;
			y = yy;
			w = ww;
			h = hh;
